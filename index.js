...
function cafe(h)
{
    for(var i=0;i<a1.length;i++)
    {
        var r = h.toUpperCase();
        var go = a1[i].name;
        var go2 = go.slice(0,h.length);
        var go3 = go2.toUpperCase();
        if(r == go3)
        {
            var place = a1[i].place_id;
            for(var j=0;j<a2.length;j++)
            {
                if(place == a2[j].id)
                {
                    console.log("name: "+a1[i].name);
                    console.log("street_no: "+a2[j].street_no);
                    console.log("locality: "+a2[j].locality);
                    console.log("postal_code: "+a2[j].postal_code);
                    console.log("lat: "+a2[j].lat);
                    console.log("long: "+a2[j].long);
                    console.log(" ");
                }
            }
        }
